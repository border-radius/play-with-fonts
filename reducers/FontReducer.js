...
import { getSubsetList, getFamilyList, getFontList } from '../data/fonts'
  ...
  useReducer(
    (oldState, diff) => {
      ...
      const family = diff.family
        ? diff.family
        : families.indexOf(oldState.family) > -1
        ? oldState.family
        : families[0]
      const fonts =
        diff.subset || diff.family ? getFontList(family) : oldState.fonts
      const font = diff.font
        ? diff.font
        : fonts.indexOf(oldState.font) > -1
        ? oldState.font
        : fonts[0]
